...
//this function calculate a 9 length prod
inline void tensorprod(paratype* graph,paratype* convweight,paratype* result, int prodnum,paratype bias=0)
{
	paratype product = 0;
   for(int inindex=0; inindex<prodnum; inindex++)
   {
	 product += graph[inindex]*convweight[inindex];
   }
   *result = product + bias;
   //printf("from%f\n", product);
}
...
//separable 1
	...
	//pointwise
	pointwiselayer_nopad(IN dep1_result, Conv1_poiW, Conv1_poiB, poi1_result, Conv1_depChannel, Conv1_depoutH, Conv1_poiKernel, Conv1_poiChannel, Conv1_poioutH, Conv1_poiStride);
	delete[] dep1_result;
	/*
	printf("weight\n");
...
//separable 2
	...
	delete[] poi1_result;
	//pointwise
	pointwiselayer_nopad(IN dep2_result, Conv2_poiW, Conv2_poiB, poi2_result, Conv2_depChannel, Conv2_depoutH, Conv2_poiKernel, Conv2_poiChannel, Conv2_poioutH, Conv2_poiStride);
	delete[] dep2_result;
//separable 3
	...
	delete[] poi2_result;
	//pointwise
	pointwiselayer_nopad(IN dep3_result, Conv3_poiW, Conv3_poiB, poi3_result, Conv3_depChannel, Conv3_depoutH, Conv3_poiKernel, Conv3_poiChannel, Conv3_poioutH, Conv3_poiStride);
	delete[] dep3_result;
//separable 4
	...
	delete[] poi3_result;
	//pointwise
	pointwiselayer_nopad(IN dep4_result, Conv4_poiW, Conv4_poiB, poi4_result, Conv4_depChannel, Conv4_depoutH, Conv4_poiKernel, Conv4_poiChannel, Conv4_poioutH, Conv4_poiStride);
	delete[] dep4_result;
//separable 5
	...
	delete[] poi4_result;
	//pointwise
	pointwiselayer_nopad(IN dep5_result, Conv5_poiW, Conv5_poiB, poi5_result, Conv5_depChannel, Conv5_depoutH, Conv5_poiKernel, Conv5_poiChannel, Conv5_poioutH, Conv5_poiStride);
	delete[] dep5_result;
//separable 6
	...
	delete[] poi5_result;
	//pointwise
	pointwiselayer_nopad(IN dep6_result, Conv6_poiW, Conv6_poiB, poi7_result, Conv6_depChannel, Conv6_depoutH, Conv6_poiKernel, Conv6_poiChannel, Conv6_poioutH, Conv6_poiStride);
	delete[] dep6_result;
...
//separable 7
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv7_poiW, Conv7_poiB, poi7_result, Conv7_depChannel, Conv7_depoutH, Conv7_poiKernel, Conv7_poiChannel, Conv7_poioutH, Conv7_poiStride);
...
//separable 8
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv8_poiW, Conv8_poiB, poi7_result, Conv8_depChannel, Conv8_depoutH, Conv8_poiKernel, Conv8_poiChannel, Conv8_poioutH, Conv8_poiStride);
...
//separable 9
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv9_poiW, Conv9_poiB, poi7_result, Conv9_depChannel, Conv9_depoutH, Conv9_poiKernel, Conv9_poiChannel, Conv9_poioutH, Conv9_poiStride);
...
//separable 10
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv10_poiW, Conv10_poiB, poi7_result, Conv10_depChannel, Conv10_depoutH, Conv10_poiKernel, Conv10_poiChannel, Conv10_poioutH, Conv10_poiStride);
...
//separable 11
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv11_poiW, Conv11_poiB, poi7_result, Conv11_depChannel, Conv11_depoutH, Conv11_poiKernel, Conv11_poiChannel, Conv11_poioutH, Conv11_poiStride);
...
//separable 12
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv12_poiW, Conv12_poiB, poi7_result, Conv12_depChannel, Conv12_depoutH, Conv12_poiKernel, Conv12_poiChannel, Conv12_poioutH, Conv12_poiStride);
...
//separable 13
	...
	//pointwise
	pointwiselayer_nopad(IN dep7_result, Conv13_poiW, Conv13_poiB, grahout, Conv13_depChannel, Conv13_depoutH, Conv13_poiKernel, Conv13_poiChannel, Conv13_poioutH, Conv13_poiStride);
	...
	for (int hi = 0; hi < Conv2outH; hi++)  //height
	{
		...
		{	
			...
			for (int c = 0; c < Conv2Channel; c++)  //channel
			{
				...
				tensorprod(onepatch, convkernel,
					Conv2out + hi * (Conv2outH*Conv2Channel) + j * Conv2Channel + c, Conv2Kernel*Conv2Kernel*graphinC, Conv2B[c]);
				//relu
	...
	for (int hi = 0; hi < depoutH; hi++)  //height
	{
		for (int j = 0; j < depoutH; j++)  //width
		{
           ...
           //do not need multi channel
				...
				tensorprod_depwise(onepatch, convkernel,
					Convout + hi * (depoutH*depChannel) + j * depChannel, depChannel,depKernel, depthwiseB);
				//relu
				relu6mul(Convout + hi * (depoutH*depChannel) + j * depChannel, depChannel);
				/*
				printf("weight\n");
				debugpr(convkernel, 27);
				...
				printf("patchzero\n");
				debugpr(Conv2out + hi * (depoutH*depChannel) + j * depChannel + c, 1);
				*/
	...
	for (int hi = 0; hi < depoutH; hi++)  //height
	{
		for (int j = 0; j < depoutH; j++)  //width
		{
			...
			for (int c = 0; c < depChannel; c++)  //channel
			{
				...
				tensorprod(onepatch, convkernel,
					Convout + hi * (depoutH*depChannel) + j * depChannel + c, depKernel*depKernel*inputc, depthwiseB[c]);
				//relu
